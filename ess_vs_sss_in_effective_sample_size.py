...
# %% functions
def pi(x):
    """ Value of the density at the point x. """
...
def special_update(x, y):
    return x + y if np.random.uniform() < pi2(x + y, x) else x

def another_update(x, y):
    return np.array(x) * np.array(y)

def random_MH_update(x0=[0], steps=10, update=None):
    d = len(x0)
    ...
    if update is None:
        update = special_update
    # apply update to noise
    return ft.reduce(update, noise)

random_MH_update([0,1], update=another_update)

def random_MH(x0=[0], steps=10):
    """ Some Metropolis-Hastings stuff apparently. """
    d = len(x0)
    # # noise = [x0, noise1, noise2, ...]. starting vector and then steps times a noise vector
    # noise = [np.array(x0)] + [np.random.normal(scale=0.04, size=d) for i in range(steps)]
    # # define the update function that is applied successively to the noise list
    ...
    #    print("Acceptance rates: mean = %.4f, sd = %.4f." %(np.mean(acceptance_rates), np.std(acceptance_rates)))
    return x0

def runiform_ball(d, R=1):
    """ Sample efficiently from a uniform distribution on a d-dimensional ball of radius R. """
...
def random_SSS(x0=[0], steps=10):
    """ Sample according to the simple slice sampler. """
    for i in range(steps):
        norm_x0 = np.linalg.norm(x0)
        t = np.random.uniform(0, np.exp(-norm_x0 - 0.5*norm_x0**2))
...
def random_two_segments(left_border, right_border, shift=np.pi):
    ...
    u = np.random.binomial(1, 0.5)
    if u == 1: x = x + shift
    return x

def ellipse_point(x1, x2, a):
    """ Returns a point of the ellipse generated by x1 and x2 with the angle a. """
    if not len(x1) == len(x2):
        sys.exit("ERROR in ellipse_point: lengths of x1 and x2 must be equal")
    return(x1 * np.cos(a) + x2 * np.sin(a))

def random_ESS(x0=[0], steps=10):
    for i in range(steps):
        # initialize many random things
        norm_x0 = np.linalg.norm(x0)
        t = np.random.uniform(0, np.exp(-norm_x0))
        w = np.random.normal(size=len(x0))
        norm_w = np.linalg.norm(w)
        # compute some constants to compute something else
        ...
        # compute another angle
        theta = random_two_segments((phi + psi) / 2, np.pi + (phi - psi) / 2)
        # compute a point on the ellipse according to the just computed angle!
